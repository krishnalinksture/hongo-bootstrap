...
if ( !isset( $post->ID ) && ! isset( $_GET['product_id'] ) ) {
	wp_die( esc_html__( 'Access Denied, No product found', 'hongo' ) );
}
...
if ( ! dokan_is_product_author( $post_id ) ) {
	wp_die( esc_html__( 'Access Denied', 'hongo' ) );
	exit();
}
					...
					<div class="dokan-dashboard-content dokan-product-edit">
						...
						<header class="dokan-dashboard-header dokan-clearfix">
							<h1 class="entry-title">
								<?php echo esc_html__( 'Edit Product', 'hongo' ); ?>
								...
								<?php if ( $post->post_status == 'publish' ) { ?>
									<span class="dokan-right">
										<a class="dokan-btn dokan-btn-theme dokan-btn-sm" href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" target="_blank"><?php echo esc_html__( 'View Product', 'hongo' ); ?></a>
									</span>
								<?php } ?>

								<?php if ( $_visibility == 'hidden' ) { ?>
									<span class="dokan-right dokan-label dokan-label-default dokan-product-hidden-label"><i class="fa fa-eye-slash"></i> <?php echo esc_html__( 'Hidden', 'hongo' ); ?></span>
								<?php } ?>
							</h1>
						</header><!-- .entry-header -->
						...
						<div class="product-edit-new-container product-edit-container">
							<?php if ( dokan()->dashboard->templates->products->has_errors() ) { ?>
								<div class="dokan-alert dokan-alert-danger">
									...
									<?php foreach ( dokan()->dashboard->templates->products->get_errors() as $error ) { ?>
										<strong><?php echo esc_html__( 'Error!', 'hongo' ); ?></strong> <?php echo esc_html( $error ); ?>.<br>
									<?php } ?>
								</div>
							<?php } ?>
							...
							<?php if ( isset( $_GET['message'] ) && $_GET['message'] == 'success') { ?>
								<div class="dokan-message">
									...
									<strong><?php echo esc_html__( 'Success!', 'hongo' ); ?></strong> <?php echo esc_html__( 'The product has been saved successfully.', 'hongo' ); ?>

									<?php if ( $post->post_status == 'publish' ) { ?>
										<a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>" target="_blank"><?php echo esc_html__( 'View Product &rarr;', 'hongo' ); ?></a>
									<?php } ?>
								</div>
							<?php } ?>
								...
								if ( dokan_is_seller_enabled( get_current_user_id() ) ) { ?>
									<form class="dokan-product-edit-form" role="form" method="post">
										...
										<div class="dokan-form-top-area">
											...
											<div class="content-half-part dokan-product-meta">
												...
												<div class="dokan-form-group">
													...
													<label for="post_title" class="form-label"><?php echo esc_html__( 'Title', 'hongo' ); ?></label>
													<?php dokan_post_input_box( $post_id, 'post_title', array( 'placeholder' => __( 'Product name..', 'hongo' ), 'value' => $post_title ) ); ?>
													<div class="dokan-product-title-alert dokan-hide">
														<?php echo esc_html__( 'Please enter product title!', 'hongo' ); ?>
													</div>
												</div>
												...
												<?php if ( is_array( $product_types ) ): ?>
													<div class="dokan-form-group">
														<label for="product_type" class="form-label"><?php echo esc_html__( 'Product Type', 'hongo' ); ?> <i class="fa fa-question-circle tips" aria-hidden="true" data-title="<?php echo esc_attr__( 'Choose Variable if your product has multiple attributes - like sizes, colors, quality etc', 'hongo' ); ?>"></i></label>
												...
												<div class="show_if_simple dokan-clearfix">
													...
													<div class="dokan-form-group dokan-clearfix dokan-price-container">
														...
														<div class="content-half-part regular-price">
															<label for="_regular_price" class="form-label"><?php echo esc_html__( 'Price', 'hongo' ); ?>
																<span
																	...
																	data-product-id="<?php echo esc_attr( $post_id ); ?>">
																		( <?php echo esc_html__( ' You Earn : ', 'hongo' ); ?><?php echo esc_html( get_woocommerce_currency_symbol() ); ?>
															...
															<div class="dokan-input-group">
																...
																<?php dokan_post_input_box( $post_id, '_regular_price', array( 'class' => 'dokan-product-regular-price', 'placeholder' => __( '0.00', 'hongo' ) ), 'price' ); ?>
															</div>
														</div>
			
														<div class="content-half-part sale-price">
															<label for="_sale_price" class="form-label">
																<?php echo esc_html__( 'Discounted Price', 'hongo' ); ?>
																<a href="#" class="sale_schedule <?php echo ($show_schedule ) ? 'dokan-hide' : ''; ?>"><?php echo esc_html__( 'Schedule', 'hongo' ); ?></a>
																<a href="#" class="cancel_sale_schedule <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php echo esc_html__( 'Cancel', 'hongo' ); ?></a>
															</label>
			
															<div class="dokan-input-group">
																<span class="dokan-input-group-addon"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></span>
																<?php dokan_post_input_box( $post_id, '_sale_price', array( 'class' => 'dokan-product-sales-price','placeholder' => __( '0.00', 'hongo' ) ), 'price' ); ?>
															</div>
														</div>
													</div>
													...
													<div class="dokan-form-group dokan-clearfix dokan-price-container">
														<div class="dokan-product-less-price-alert dokan-hide">
															<?php echo esc_html__('Product price can\'t be less than the vendor fee!', 'hongo' ); ?>
														</div>
													</div>
													...
													<div class="sale_price_dates_fields dokan-clearfix dokan-form-group <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>">
														<div class="content-half-part from">
															<div class="dokan-input-group">
																<span class="dokan-input-group-addon"><?php echo esc_html__( 'From', 'hongo' ); ?></span>
																<input type="text" name="_sale_price_dates_from" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_from ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php echo esc_attr__( 'YYYY-MM-DD', 'hongo' ); ?>">
															</div>
														</div>
			
														<div class="content-half-part to">
															<div class="dokan-input-group">
																<span class="dokan-input-group-addon"><?php echo esc_html__( 'To', 'hongo' ); ?></span>
																<input type="text" name="_sale_price_dates_to" class="dokan-form-control datepicker" value="<?php echo esc_attr( $_sale_price_dates_to ); ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" placeholder="<?php echo esc_attr__( 'YYYY-MM-DD', 'hongo' ); ?>">
															</div>
														</div>
												...
												<?php if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' ): ?>
													<div class="dokan-form-group">
														<label for="product_cat" class="form-label"><?php echo esc_html__( 'Category', 'hongo' ); ?></label>
														<?php
														$product_cat = -1;
														$term = array();
														...
														$category_args =  array(
															'show_option_none' => __( '- Select a category -', 'hongo' ),
															'hierarchical'     => 1,
															'hide_empty'       => 0,
															'name'             => 'product_cat',
															'id'               => 'product_cat',
													...
													?>
														<div class="dokan-product-cat-alert dokan-hide">
															<?php echo esc_html__('Please choose a category!', 'hongo' ); ?>
														</div>
													</div>
												<?php elseif ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'multiple' ): ?>
													<div class="dokan-form-group">
														...
														echo str_replace( '<select', '<select data-placeholder="' . esc_attr__( 'Select product category', 'hongo' ) . '" multiple="multiple" ', $drop_down_category ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
														?>
													</div>
												<?php endif; ?>
			
												<div class="dokan-form-group">
													<label for="product_tag" class="form-label"><?php echo esc_html__( 'Tags', 'hongo' ); ?></label>
													<?php
													...
													echo str_replace( '<select', '<select data-placeholder="' . esc_attr__( 'Select product tags', 'hongo' ) . '" multiple="multiple" ', $drop_down_tags ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
													?>
												</div>
											...
											<div class="content-half-part featured-image">
												...
												<div class="dokan-feat-image-upload dokan-new-product-featured-img">
													...
													<div class="instruction-inside<?php echo esc_attr( $instruction_class ); ?>">
														...
														<a href="#" class="dokan-feat-image-btn btn btn-sm"><?php echo esc_html__( 'Upload a product cover image', 'hongo' ); ?></a>
													</div>
																	...
																	if ( $gallery ) {
																		foreach ($gallery as $image_id) {
																			...
																			<li class="image" data-attachment_id="<?php echo esc_attr( $image_id ); ?>">
																				...
																				<a href="#" class="action-delete" title="<?php esc_attr_e( 'Delete image', 'hongo' ); ?>">&times;</a>
																			</li>
																			<?php
																		}
																	}
																	?>
																	<li class="add-image add-product-images tips" data-title="<?php echo esc_attr__( 'Add gallery image', 'hongo' ); ?>">
										...
										<div class="dokan-product-short-description">
											<label for="post_excerpt" class="form-label"><?php echo esc_html__( 'Short Description', 'hongo' ); ?></label>
										...
										<div class="dokan-product-description">
											<label for="post_content" class="form-label"><?php echo esc_html__( 'Description', 'hongo' ); ?></label>
										...
										<input type="hidden" name="dokan_update_product" value="<?php esc_attr_e( 'Save Product', 'hongo' ); ?>"/>
										<input type="submit" name="dokan_update_product" class="dokan-btn dokan-btn-theme dokan-btn-lg dokan-right" value="<?php esc_attr_e( 'Save Product', 'hongo' ); ?>"/>
