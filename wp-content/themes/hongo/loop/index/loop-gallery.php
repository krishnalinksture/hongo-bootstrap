 ...
 */
	...
	$hongo_srcset_default 		= get_theme_mod( 'hongo_image_srcset_default', 'full' );
...
?>
		...
		<?php if( is_array( $hongo_gallery ) ) { ?>
			<li class="grid-sizer col-lg-4 col-md-6 col-12"></li>
			<?php
				foreach ( $hongo_gallery as $key => $value ) {
					...
					$hongo_thumb 				= wp_get_attachment_image_src( $value, $hongo_srcset_default );
					$hongo_full_url 			= wp_get_attachment_image_src( $value, 'full' );
					if( $hongo_thumb[0] ) {

						// Get Image Srcset and Sizes
			            $srcset_data = hongo_get_image_srcset_sizes( $value, $hongo_srcset_default );
			?>

	                	<li class="col-lg-4 col-md-6 col-12 grid-item">
   	...
   	<?php  } else { ?>
		<div class="blog-image post-type-slider">
