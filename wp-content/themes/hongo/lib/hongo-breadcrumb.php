...
if( ! class_exists('Hongo_Breadcrumb_Navigation') ){
	class Hongo_Breadcrumb_Navigation {
		...
		function __construct() {
			$this->opt = array(
				...
				// Text displayed for the search page.
					'title_search' => esc_html__( 'Search', 'hongo' ),
				...
				// The prefix that is used for mouseover link (e.g.: "Browse to: Archive")
					'urltitle_prefix' => esc_html__( 'Browse to: ', 'hongo' ),
	...
	/*==============================================================================
	    				=== DISPLAY BREADCRUMB ===
	  ============================================================================*/
		function hongo_display_breadcrumb() {
			...
			************************************************* */
			...
			case 'page':
				...
				while( 0 != $bcn_theparentid ) {
					...
					$bcn_titlearray[ $bcn_loopcount ] = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" class="text-dark-gray-2" href="' . esc_url( get_permalink( $bcn_theparentid ) ) . '" title="' . $this->opt['urltitle_prefix'] . $mylink->post_title . $this->opt['urltitle_suffix'] . '"><span itemprop="name">' . $mylink->post_title . '</span><meta itemprop="position" content="'.$page_counter.'" /></a></li>';
			...
			$result_array = apply_filters( 'hongo_breadcrumb_before_dispaly_args', $result_array );
	...
	} // END class Hongo_Breadcrumb_Navigation
}
