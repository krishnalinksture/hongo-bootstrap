 ...
 * Title section
 *
 * This template can be overridden by copying it to yourchildtheme/templates/title/title-style-8.php
 */
...
$hongo_poster_image = ( $hongo_title_bg_image_url ) ? ' poster="' . esc_url( $hongo_title_bg_image_url ) . '"': '';
...
echo '<section class="hongo-main-title-wrap bg-very-light-gray' . esc_attr( $hongo_title_class.'-wrap ' ) . esc_attr( $hongo_title_style ) . esc_attr( $hongo_title_parallax ) . esc_attr( $hongo_space_class ) . '" ' . $hongo_title_parallax_effect . $hongo_title_bg_image . '>'; // @codingStandardsIgnoreLine
	...
	?>
		<div class="<?php echo esc_attr( $container_style ); ?>">
			<div class="row">
				<div class="col-md-12 col-sm-12 col-xs-12 center-col display-table page-title-large<?php echo esc_attr( $content_height_class ); ?>">
	...
	<?php } ?>
	<?php if( $hongo_title_video_type == 'self' && ( $hongo_title_video_mp4 || $hongo_title_video_ogg || $hongo_title_video_webm )) { ?>
		<?php echo '<video playsinline autoplay'.$hongo_loop_video.$hongo_mute_video.' class="html-video z-index-1"'.$hongo_poster_image.'>';  // @codingStandardsIgnoreLine ?>
			<?php if( $hongo_title_video_mp4 ) { ?>
				<source type="video/mp4" src="<?php echo esc_url( $hongo_title_video_mp4 ); ?>" />
			<?php } ?>
			<?php if( $hongo_title_video_ogg ) { ?>
				<source type="video/ogg" src="<?php echo esc_url( $hongo_title_video_ogg ); ?>" />
			<?php } ?>
			<?php if( $hongo_title_video_webm ) { ?>
				<source type="video/webm" src="<?php echo esc_url( $hongo_title_video_webm ); ?>" />
			<?php } ?>
		</video>
	<?php } elseif( $hongo_title_video_type == 'external' && ( $hongo_title_video_youtube )) { ?>
		<div class="external-fit-videos fit-videos width-100">
			<iframe width="540" height="315" src="<?php echo esc_url( $hongo_title_video_youtube ); ?>" allowfullscreen   allow="autoplay; fullscreen"></iframe>
		</div>
	<?php } ?>
</section>
