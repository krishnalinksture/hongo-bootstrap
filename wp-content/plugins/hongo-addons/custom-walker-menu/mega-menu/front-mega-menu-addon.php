...
/**
 * Navigation Menu API: Hongo_Mega_Menu_Walker class
 * Hongo Front Menu Walker Class.
 *
 */

if( ! class_exists( 'Hongo_Mega_Menu_Walker' ) ) {

    class Hongo_Mega_Menu_Walker extends Walker_Nav_Menu {
        ...
        public function start_lvl( &$output, $depth = 0, $args = array() ) {
            ...
            if( $depth == 0 ) {
                if( ! empty( $hongo_mega_menu_section ) ) {
                    $output .= '<div class="menu-wrap-div megamenu-content mega-menu collapse mega-menu-full" >';
                    $classes[] = 'equalize sm-equalize-auto';
                } else {
                    $output .= '<div class="menu-wrap-div simple-menu">';
                }
            }
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if( $depth == 0 ) {
                $title = '<span class="menu-hover-line">' . $title . '</span>';
            }


            $link_class = apply_filters( 'hongo_main_link_css_class', $link_class, $depth );
            ...
            if( $hongo_mega_menu_enable_icon_image_status == 'enabled' ) {
                ...
                $item_output .= '<a'. $attributes .' itemprop="url" '.$link_class_name.'>';
                    ...
                    if( ! empty( $hongo_mega_menu_icon_image_url ) ) {
                        $custom_image = '<img class="left-icon menu-link-icon" alt="' . esc_attr__( 'Icon', 'hongo-addons' ) . '" src="'.esc_url( $hongo_mega_menu_icon_image_url ).'">';                        
                    }
                    
                    $item_output .= $args->link_before . $custom_image . $title . $args->link_after;
                ...
                $item_output .= '<a'. $attributes .' itemprop="url" '.$link_class_name.'>';
                    ...
                    if( ! empty( $hongo_menu_item_icon ) && $hongo_menu_item_icon != 1 ) {
                        $custom_icon = '<i class="'.esc_attr( $hongo_menu_item_icon ).esc_attr( $link_class ).' left-icon submenu-icon"></i>';                        
                    }
                    $item_output .= $args->link_before . $custom_icon . $title . $args->link_after;
            ...
            if( ( $args->walker->has_children && $depth == 0 ) || ! empty( $hongo_mega_menu_section ) ) {
                $item_output .= '<i class="fa-solid fa-angle-down dropdown-toggle" data-toggle="dropdown"></i>';
            }
            ...
            if( ! empty( $hongo_mega_menu_section ) ) {
                ...
                if ( ! empty( $hongo_mega_menu_post_object ) && $hongo_mega_menu_post_object->post_status == 'publish' ) {
                    $vc_custom_css = $hongo_megamenu_background_color = $hongo_megamenu_background_image = '';
                    ...
                    $item_output .= '<div class="menu-wrap-div mega-menu-main-wrapper" >';
