	...
	function hongo_product_lists_shortcode( $atts, $content = null ) {
		...
		if( $hongo_enable_slider == '1' ) {
	        ...
	        if( hongo_load_javascript_by_key( 'swiper' ) ) {
		        ob_start(); ?>
		        	$( '#<?php echo sprintf( '%s', $hongo_slider_id ); ?> .woocommerce' ).addClass( 'swiper' ); $( '#<?php echo sprintf( '%s', $hongo_slider_id ); ?> .products' ).addClass( 'swiper-wrapper' ); $( '#<?php echo sprintf( '%s', $hongo_slider_id ); ?> .product' ).addClass( 'swiper-slide' ); var <?php echo str_replace( '-', '_', $hongo_slider_id ); ?>;<?php echo str_replace( '-', '_', $hongo_slider_id ); ?> = new Swiper('#<?php echo sprintf( '%s', $hongo_slider_id ); ?> .woocommerce', { <?php echo sprintf( '%s', $slider_config ); ?> }); $( '.nav-tabs li a' ).on('click', function() { setTimeout(function () { if ($( '#<?php echo sprintf( '%s', $hongo_slider_id ); ?>' ).length > 0){ <?php echo str_replace( '-', '_', $hongo_slider_id ); ?>.update(); } }, 500); }); 
		        <?php
